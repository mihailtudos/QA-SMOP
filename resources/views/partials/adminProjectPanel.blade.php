                ...
                <div >
                    <nav class="d-flex justify-content-center mb-3">
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            <a class="nav-item nav-link  text-dark font-weight-bold @if(session()->has('meeting') or session()->has('form') or session()->has('attendance') or session()->has('form') or session()->has('upcoming') or session()->has('cancelled'))   @else active @endif " id="nav-tasks-tab" data-toggle="tab" href="#nav-tasks" role="tab" aria-controls="nav-tasks" aria-selected="false">Tasks</a>
                            ...
                            <a class="nav-item nav-link text-dark font-weight-bold @if(session()->has('meeting') or session()->has('attendance') or session()->has('upcoming') or session()->has('cancelled')) active  @else  @endif" id="nav-details-tab" data-toggle="tab" href="#nav-details" role="tab" aria-controls="nav-details" aria-selected="false">Meetings</a>
                    ...
                    <div class="tab-content p-3" id="nav-tabContent">
                        <div class="tab-pane fade @if(session()->has('meeting') or session()->has('form') or session()->has('attendance') or session()->has('upcoming')  or session()->has('cancelled'))  @else show active @endif" id="nav-tasks" role="tabpanel" aria-labelledby="nav-tasks-tab">
                            @if($form != null )
                                @if($form->approved)
                        ...
                        <div class="tab-pane fade @if(session()->has('meeting') or session()->has('attendance') or session()->has('upcoming') or session()->has('cancelled')) show active  @else @endif" id="nav-details" role="tabpanel" aria-labelledby="nav-details-tab">
